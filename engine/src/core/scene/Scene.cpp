...
namespace Paper {
	...
	void Scene::Render()
	{
		...
		if (config.show2DColliders)
		{
			//physics collider
			auto view = registry.view<TransformComponent, Collider2DComponent>();
			for (auto [entity, transform, collider] : view.each())
			{
				glm::vec4 color = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
				if (collider.geometry == Geometry::CIRCLE)
				{
					CircleRenderData data;
					glm::vec3 position = transform.position + glm::vec3(collider.offset, 0.0f);
					glm::vec3 scale = transform.scale * glm::vec3(collider.radius * 2.03, collider.radius * 2.03, 1.0f);

					data.color = color;
					data.transform = glm::translate(glm::mat4(1.0f), position)
						* glm::scale(glm::mat4(1.0f), scale);

					data.thickness = 0.02f;
					data.fade = 0.0f;
					Renderer2D::DrawCircle(data);
				}
				else
